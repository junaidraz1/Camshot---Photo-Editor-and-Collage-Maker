...
class StickerTabAdapter(context: Context) : RecyclerView.Adapter<StickerTabAdapter.TabHolder>() {
    ...
    override fun onBindViewHolder(holder: TabHolder, position: Int) {
        if (selected == position) {
            holder.ll_color.setBackgroundColor(mcontext.resources.getColor(R.color.light_purple))
        } else {
            holder.ll_color.setBackgroundColor(mcontext.resources.getColor(R.color.greyishBlack))
        }
